...
function lastMonthCompanyTotalSleep() {
    return knex('entries')
        ...
        .andWhere('date', '>=', knex.raw("(CURRENT_DATE - INTERVAL '30 days')"))
        .then((createdRecords) => {
            createdRecords[0].sleep_duration_total = Number(
                createdRecords[0].sleep_duration_total
            )
...
function lastMonthCompanySleepQualityAverage() {
    return knex('entries')
        ...
        .andWhere('date', '>=', knex.raw("(CURRENT_DATE - INTERVAL '30 days')"))
        .then((createdRecords) => {
            createdRecords[0].quality_of_sleep_average = Number(
                createdRecords[0].quality_of_sleep_average
            )
            return createdRecords[0]
        })
        .catch((error) => {
